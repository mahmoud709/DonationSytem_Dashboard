...
export default function GiveCoins() {
   ...
   const { id } = useParams();
   const [coins, setCoinsValue] = useState({
      coins: "",
   })
   ...
   async function addCoins() {
      const { data } = await axios.post(`${API_LINK}/coins/givecoins/${id}`, coins);
      if (data.success) {
         alert(data.message);
         navigate("/orders")
      }
   ...
   return (
      <div className='col-md-9' onSubmit={handleSubmit}>
         ...
         <form action="" className="my-3">
            ...
            <input onChange={takeCoinsValue} type='text' name='coins' placeholder='value' className='form-control w-50 mb-3' />
